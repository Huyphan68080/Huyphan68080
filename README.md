<div align="center">

  <!-- Header Banner / Animated Typing -->
  <h1><img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="30px"> Hey there, I'm <a href="https://github.com/HuyPhan68080">Huy Phan</a>!</h1>
  
  <h3>💻 Fullstack Web Developer | Python • JavaScript • TypeScript</h3>

  <p align="center">
    <a href="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=20&duration=2500&color=00F7FF&center=true&vCenter=true&width=550&lines=Fullstack+Developer;ReactJS+%2B+NodeJS+%2B+Flask;Building+Real-World+Web+Apps;Passion+For+Clean+Code">
      <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=20&duration=2500&color=00F7FF&center=true&vCenter=true&width=550&lines=Fullstack+Developer;ReactJS+%2B+NodeJS+%2B+Flask;Building+Real-World+Web+Apps;Passion+For+Clean+Code" alt="Typing SVG" />
    </a>
  </p>

  <!-- Quick Badges -->
  <p align="center">
    <img src="https://img.shields.io/badge/Focus-Fullstack_Web_Development-00f7ff?style=for-the-badge&logoColor=white" />
    <img src="https://img.shields.io/badge/Learning-Advanced_System_Architecture-ff007f?style=for-the-badge&logoColor=white" />
    <img src="https://img.shields.io/badge/Status-Open_To_Work-00ff7f?style=for-the-badge&logoColor=white" />
  </p>

</div>

<br/>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />

<br/>

## 🚀 About Me

- 🧑‍💻 **Fullstack Web Developer** passionate about crafting modern, responsive, and robust web applications.
- ⚡ Experienced in building end-to-end **E-commerce platforms**, **authentication systems**, and **interactive UI dashboards**.
- 💡 Always exploring modern web technologies, performance optimization, and clean architecture.

<br/>

### 🎯 Key Highlights

- ⚛️ **Frontend:** Building interactive & scalable interfaces using **React.js**, **TypeScript**, & **Tailwind CSS**.
- ⚙️ **Backend:** Crafting reliable RESTful APIs with **Node.js (Express)** & **Python (Flask)**.
- 🗄️ **Databases:** Designing relational & NoSQL schemas with **MySQL** & **MongoDB**.
- 🛒 **Domain:** Specialized in E-commerce systems (Cart, Checkout, Role-based Auth, Admin Dashboards).

<br/>

---

## 🧠 Tech Stack & Skills

<h4 align="center">🌐 Frontend Development</h4>
<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=html,css,js,ts,react,tailwind,bootstrap&perline=7" />
  </a>
</p>

<h4 align="center">⚙️ Backend & Database</h4>
<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=nodejs,express,py,flask,mongodb,mysql,postman&perline=7" />
  </a>
</p>

<h4 align="center">🛠️ Tools & Environment</h4>
<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=git,github,vscode,figma,powershell&perline=7" />
  </a>
</p>

<br/>

---

## 📂 Featured Projects

| Project | Description | Tech Stack |
| :--- | :--- | :--- |
| 🛒 **Fruit E-commerce Website** | Full-featured online store with product browsing, shopping cart, and checkout system. | `React` `Tailwind` `Node.js` `MongoDB` |
| 🔐 **Auth & Role Management System** | Robust authentication system supporting Admin/User roles and permission management. | `React` `Express` `JWT` `MySQL` |
| 📦 **Product Management Dashboard** | Comprehensive admin panel for inventory, analytics, and product management. | `React` `TypeScript` `Flask` `MySQL` |
| 🎨 **UI Animation Web** | Interactive web experience featuring rich visual animations and micro-interactions. | `HTML5` `CSS3` `JavaScript` `Framer` |

<br/>

---

## 📈 GitHub Statistics & Activity

<div align="center">
  <a href="https://github.com/HuyPhan68080">
    <img height="180em" src="https://github-readme-stats.vercel.app/api?username=HuyPhan68080&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true"/>
    <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=HuyPhan68080&layout=compact&theme=tokyonight&hide=html,css"/>
  </a>
</div>

<br/>

<div align="center">
  <a href="https://github.com/HuyPhan68080">
    <img src="https://streak-stats.demolab.com?user=HuyPhan68080&theme=tokyonight&hide_border=false"/>
  </a>
</div>

<br/>

<div align="center">
  <a href="https://github.com/HuyPhan68080">
    <img width="90%" src="https://github-readme-activity-graph.vercel.app/graph?username=HuyPhan68080&theme=tokyo-night"/>
  </a>
</div>

<br/>

---

## 🏆 GitHub Trophies

<p align="center">
  <a href="https://github.com/HuyPhan68080">
    <img src="https://github-profile-trophy.vercel.app/?username=HuyPhan68080&theme=tokyonight&no-frame=false&no-bg=false&margin-w=15"/>
  </a>
</p>

<br/>

---

## 📬 Connect & Contact

<p align="center">
  <a href="https://github.com/HuyPhan68080" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  &nbsp;
  <a href="mailto:contact@example.com" target="_blank">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  &nbsp;
  <a href="https://linkedin.com" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
</p>

<br/>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=auto&height=120&section=footer"/>
</p>
